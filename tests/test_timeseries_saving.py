...
def test_create_unique_timeseries(app, basic_tstype):
    ...
    with app.app_context():
        ...
        assert TimeSeries.query.count() == 1, "Exactly one TimeSeries should exist in the database."
...
def test_timeseries_conflict_no_allow_update_same_name(app, basic_tstype):
    ...
    """
    with app.app_context():
        existing_ts = TimeSeries(name="TS_Conflict", time_series_type=basic_tstype, code="CONF001")
        existing_ts.save()

        ts_conflict = TimeSeries(name="TS_Conflict", time_series_type=basic_tstype, code="NEWCODE")
        with pytest.raises(ValueError) as exc_info:
            ts_conflict.save(allow_update=False)

        assert "TimeSeries with the same name" in str(exc_info.value), \
...
def test_timeseries_conflict_no_allow_update_same_code(app, basic_tstype):
    ...
    """
    with app.app_context():
        existing_ts = TimeSeries(name="TS_Other", time_series_type=basic_tstype, code="DUPLICATE")
        existing_ts.save()

        ts_conflict = TimeSeries(name="TS_NewName", time_series_type=basic_tstype, code="DUPLICATE")
        with pytest.raises(ValueError) as exc_info:
            ts_conflict.save(allow_update=False)

        assert "TimeSeries with the same code" in str(exc_info.value), \
...
def test_timeseries_conflict_with_allow_update_same_name(app, basic_tstype):
    ...
    """
    with app.app_context():
        existing_ts = TimeSeries(name="TS_Merge", time_series_type=basic_tstype, code="MERGE001")
        existing_ts.description = "Original description"
        existing_ts.save()

        # Create a new one with the same name, different code
        ts_merge = TimeSeries(name="TS_Merge", time_series_type=basic_tstype, code="MERGEXXX")
        ts_merge.description = "New description"
        ts_merge.save(allow_update=True, keep_old_description=False)  # We do NOT keep the old description

        # The existing record's ID should be reused
        assert ts_merge.id == existing_ts.id, "Should merge with the existing record."
        assert ts_merge.description == "New description", "Should replace the old description."
...
def test_timeseries_conflict_with_allow_update_same_code(app, basic_tstype):
    ...
    """
    with app.app_context():
        existing_ts = TimeSeries(name="TS_MergeCode", time_series_type=basic_tstype, code="CODE001")
        existing_ts.delta_type = "pct"
        existing_ts.save()

        # Create a new one with a different name, same code
        ts_merge_code = TimeSeries(name="TS_NewName", time_series_type=basic_tstype, code="CODE001")
        ts_merge_code.delta_type = "abs"
        ts_merge_code.save(allow_update=True, keep_old_delta_type=False)  # We do NOT keep the old delta_type

        # The existing record's ID should be reused
        assert ts_merge_code.id == existing_ts.id, "Should merge with the existing record."
...
def test_timeseries_integrity_error(app, basic_tstype):
    ...
    """
    with app.app_context():
        ts = TimeSeries(name="TS_Integrity", time_series_type=basic_tstype, code="INTG001")
        ts.save()
        ...
        try:
            duplicate_ts = TimeSeries(name="TS_Integrity", time_series_type=basic_tstype, code="INTG001")
        ...
        assert TimeSeries.query.count() == 1, "There should still be only one TimeSeries in the DB."
...
def test_join_data_points_no_duplicates(app, create_timeseries):
    ...
    dp2 = DataPoint(date=datetime.date(2025, 5, 5), value=100.0)  # duplicate date & value
    dp3 = DataPoint(date=datetime.date(2025, 5, 6), value=100.0)  # same value, different date
    dp4 = DataPoint(date=datetime.date(2025, 5, 6), value=200.0)  # same date, different value
...
def test_merge_existing_data_points_on_update(app, basic_tstype):
    ...
    """
    with app.app_context():
        old_ts = TimeSeries(name="TS_MergeDP", time_series_type=basic_tstype, code="MERGEDP")
        old_ts.save()
        ...
        new_ts = TimeSeries(name="TS_MergeDP", time_series_type=basic_tstype, code="NEWMERGE")
        ...
        assert date_val_set == expected_set, "Merged set of DataPoints should match expected."